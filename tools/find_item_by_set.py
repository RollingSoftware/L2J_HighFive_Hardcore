...
script_name, workdir, file_extension, recursive, attribute, value, item_type = sys.argv
...
for item in files_to_process:
    ...
    for element in item_elements:
        if element.getAttribute('type') in item_type.split(','):
            sets = element.getElementsByTagName('set')
            for one_set in sets:
                if attribute == one_set.getAttribute('name') and one_set.getAttribute('val') in value.split(','):
