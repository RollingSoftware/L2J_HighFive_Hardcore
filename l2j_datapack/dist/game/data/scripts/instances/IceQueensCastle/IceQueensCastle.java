...
public final class IceQueensCastle extends AbstractInstance {
        ...
        switch (event) {
            case "ATTACK_KNIGHT": {
                for (L2Character character : npc.getKnownList().getKnownCharacters()) {
                    if ((character.getId() == ARCHERY_KNIGHT) && !character.isDead() && !((L2Attackable) character)
                        .isDecayed()) {
    ...
    public String onSeeCreature(L2Npc npc, L2Character creature, boolean isSummon) {
        if (creature.isPlayer() && npc.isScriptValue(0)) {
            ...
            broadcastNpcSay(npc, Say2.NPC_ALL, NpcStringId.S1_MAY_THE_PROTECTION_OF_THE_GODS_BE_UPON_YOU,
                creature.getName());
        }
    ...
    public String onSpellFinished(L2Npc npc, L2PcInstance player, Skill skill) {
        ...
        if (tmpworld instanceof IQCWorld) {
